...
| | |
|---|---|
| Version | **0.105.3** (extension) — **text comparisons use their indexes inside functions called with a `name` argument** (#35): PL/pgSQL gave such functions collation "C", so comparisons against the extension's default-collation indexes fell back to scanning — the dirty-queue purge fired per deleted statement read the whole queue, and memory search filtered every compartment of a schema; 17 functions now compare with `COLLATE "default"`, and the `collation_index_use` regress test lists any comparison with that shape. On 0.105.2 **bulk deletes no longer quadratic** (#33): indexes on `malu$svpor_statement.source_package_id`, `malu$community_membership.community_id` and `malu$ann_delta.chunk_id`, so deleting memory data, replacing communities and deleting ANN-indexed compartments no longer scan a child table per row; a new `fk_index_coverage` regress test lists every foreign key still without a usable index. On 0.105.1 **`malu_vector` text output reads back exactly** (#31): elements print as the shortest decimal that reproduces the float4, so `pg_dump`, `COPY` and client reads no longer round embeddings. On 0.105.0 **`pg_dump` carries MaluDB's data** (#27): data tables are registered with `pg_extension_config_dump`, so logical backups and dump/restore migrations keep what MaluDB stores and none of the rows the extension installs; the per-database master key and auth pepper are deliberately not dumped, and restores should run with `session_replication_role=replica` (see CHANGELOG). On 0.104.0 **relational data-model graph (DM-1)**: database objects become a graph namespace, so coding tools can ask "how do I join A to B" / "what breaks if I change X" through the same surface as the code graphs — `maludb_datamodel_refresh` introspects `pg_catalog` (tables/views/matviews with columns + PK attributes, routines, non-internal triggers, one node per schema) into node-link form and imports it via the 0.103.0 graph import: `fk_references`/`depends_on`/`triggers_on`/`belongs_to` edges as EXTRACTED, routine reads/writes via a coarse `prosrc` scan as INFERRED, visibility limited to the tenant's own schema plus `maludb_core` and `public`. Companion `maludb_datamodel_describe` returns live columns/PK/FKs-in-and-out for one relation, and graph import gains `options.resolve_external` (link endpoints matching an existing subject canonical name resolve — the cross-namespace stitch for code mining). On 0.103.0 **in-core graph import**: one-call `maludb_graph_import(namespace, graph jsonb, options jsonb)` — validates the namespace and caps (50k nodes / 200k links), registers node types idempotently (`concept` fallback), maps nodes → subjects (canonical `<ns>/<id>`, label alias, `graphify_*` attributes) and links → SVO edges (EXTRACTED/INFERRED/AMBIGUOUS → confidence 1.0/0.7/0.4) in one ingest call with per-item subtransaction isolation (unknown endpoints reported in `skipped`, not fatal); node community tags stored via community replace. Graph import is core functionality — every API server exposes it as a thin wrapper. On 0.102.0 **graph communities + analytics + subject-type registration**: `malu$community`/`malu$community_membership` (namespace-scoped community sets over graph objects, replace semantics per namespace; clustering itself stays client-side), facade `maludb_community_replace`, `uedge_degree` (in/out/total degree over `malu$edge_unified` — the god-nodes query), `uedge_surprises` (cross-community edges ranked by community-pair rarity), and tenant-callable idempotent `maludb_register_subject_type` into the global subject-type catalog. On 0.101.0 **unified-graph path finding**: `uedge_path(source, target, max_depth, direction, rel[])` over `malu$edge_unified`, shortest path first, direction defaulting to `both`, depth capped [1,32], with tenant facade `maludb_graph_path`. On 0.100.0 **document/note reindex protocol**: a background worker re-derives a document's or note's SVPOR graph footprint (subjects/verbs/SVO statements) against the current knowledge graph — `maludb_memory_reindex_claim` (stalest-first, registry-aware) → `maludb_memory_reindex_apply` (replace the `$source`-anchored statement footprint, then re-ingest idempotently), with `last_indexed`/`last_indexed_model` watermarks on `malu$document`; the documents/notes analogue of the 0.99.0 skill reindex, and likewise core never calls a model. Latest release tag `v4.5.0` shipped extension 0.100.0 on 2026-06-17; 0.101.0–0.105.0 are on `main` ahead of the next release tag. V4 acceptance suite: `scripts/maludb-fieldtest-v4` walks every V4 surface end-to-end; `bench/v4/run-bench` publishes recall + latency baselines; `docs/v4/acceptance-matrix.md` maps plan §12 criteria to test artefacts. |
| Test suite | **97 pg_regress targets** on PG 17 plus restd, realtimed, CLI, libmaludb v0.2, and pageindexd parser smoke checks |
...
You should see exactly: `0.104.0`
